...
function woocommerce_product_custom_fields()
{
	...
	woocommerce_wp_text_input(
        array(
            'id' => 'Year_of_Publication_text_field',
            'placeholder' => 'Year of Publication',
            'label' => __('Year of Publication', 'woocommerce'),
            'desc_tip' => 'true'
        )
    );
	woocommerce_wp_text_input(
        array(
            'id' => 'Subject_text_field',
            'placeholder' => 'Subject',
	...
	woocommerce_wp_text_input(
        array(
            'id' => 'Place_text_field',
            'placeholder' => 'Place',
            'label' => __('Place', 'woocommerce'),
            'desc_tip' => 'true'
        )
    );
	woocommerce_wp_text_input(
        array(
            'id' => 'Edition_text_field',
            'placeholder' => 'Edition',
            'label' => __('Edition', 'woocommerce'),
            'desc_tip' => 'true'
        )
    );
...
{
    // Custom Product Text Field
    $woocommerce_custom_product_text_field = $_POST['Author_text_field'];
    if (!empty($woocommerce_custom_product_text_field))
        update_post_meta($post_id, 'Author_text_field', esc_attr($woocommerce_custom_product_text_field));

	$woocommerce_custom_product_text_field = $_POST['Publisher_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
	update_post_meta($post_id, 'Publisher_text_field', esc_attr($woocommerce_custom_product_text_field));
	
	$woocommerce_custom_product_text_field = $_POST['Number_of_Pages_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
		update_post_meta($post_id, 'Number_of_Pages_text_field', esc_attr($woocommerce_custom_product_text_field));
	
	$woocommerce_custom_product_text_field = $_POST['Year_of_Publication_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
		update_post_meta($post_id, 'Year_of_Publication_text_field', esc_attr($woocommerce_custom_product_text_field));
	
	$woocommerce_custom_product_text_field = $_POST['Subject_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
			update_post_meta($post_id, 'Subject_text_field', esc_attr($woocommerce_custom_product_text_field));
	
	$woocommerce_custom_product_text_field = $_POST['Place_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
		update_post_meta($post_id, 'Place_text_field', esc_attr($woocommerce_custom_product_text_field));
	
	$woocommerce_custom_product_text_field = $_POST['Edition_text_field'];
	if (!empty($woocommerce_custom_product_text_field))
		update_post_meta($post_id, 'Edition_text_field', esc_attr($woocommerce_custom_product_text_field));
	
}
add_filter( 'woocommerce_product_add_to_cart_text', 'woocommerce_custom_product_add_to_cart_text' );  
function woocommerce_custom_product_add_to_cart_text() {
    return __( 'Adopt a Book', 'woocommerce' );
}
...
function cw_add_postmeta_ordering_args( $args_sort_cw ) {
	...
	switch( $cw_orderby_value ) {
		 case 'Year_of_Publication_text_field':
			  $args_sort_cw['orderby'] = 'meta_value';
			  $args_sort_cw['order'] = 'DESC';
			  $args_sort_cw['meta_key'] = 'Year_of_Publication_text_field';
			  break;
			  case 'Subject_text_field':
				$args_sort_cw['orderby'] = 'meta_value';
				$args_sort_cw['order'] = 'DESC';
				$args_sort_cw['meta_key'] = 'Subject_text_field';
				break;
			  case 'name':
				$args_sort_cw['orderby'] = 'title';
				$args_sort_cw['order'] = 'DESC';
   ...
   function cw_add_new_postmeta_orderby( $sortby ) {
	$sortby['name'] = __( 'Sort By Title', 'woocommerce' );
	$sortby['Subject_text_field'] = __( 'Sort By Subject', 'woocommerce' );
	$sortby['Year_of_Publication_text_field'] = __( 'Sort By Year of Publication', 'woocommerce' );
